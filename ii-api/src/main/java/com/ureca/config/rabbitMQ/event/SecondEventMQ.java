...
@Configuration
public class SecondEventMQ extends EventRabbitMQConfig {
  ...
  @Bean
  public Queue SecondEventQueue() {
    return this.createQueue(QUEUE_NAME, DEAD_LETTER_EXCHANGE);
  }

  @Bean
  public Binding SecondEventBinding(Queue SecondEventQueue, DirectExchange eventExchange) {
    return BindingBuilder.bind(SecondEventQueue).to(eventExchange).with(ROUTING_KEY);
  }
  ...
  public CommandLineRunner initializeEventQueue(
    ...
    return args -> {
      rabbitAdmin.declareQueue(SecondEventQueue());
      rabbitAdmin.declareExchange(this.eventExchange());
      rabbitAdmin.declareBinding(SecondEventBinding(SecondEventQueue(), this.eventExchange()));
    };
  }
}
