...
@Configuration
public class ThirdEventMQ extends EventRabbitMQConfig {

  private static final int QUEUE_NUMBER = 3;
  ...
  @Bean
  public Queue ThirdEventQueue() {
    return this.createQueue(QUEUE_NAME, DEAD_LETTER_EXCHANGE);
  }

  @Bean
  public Binding ThirdEventBinding(Queue ThirdEventQueue, DirectExchange eventExchange) {
    return BindingBuilder.bind(ThirdEventQueue).to(eventExchange).with(ROUTING_KEY);
  }
  ...
  public CommandLineRunner initializeEventQueue(
    ...
    return args -> {
      rabbitAdmin.declareQueue(ThirdEventQueue());
      rabbitAdmin.declareExchange(this.eventExchange());
      rabbitAdmin.declareBinding(ThirdEventBinding(ThirdEventQueue(), this.eventExchange()));
    };
  }
}
